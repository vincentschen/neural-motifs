...
parser.add_argument('--data-prefix', type=str, default='', 
                    help='prefix for data we are using')
...
for _ in range(args.nruns):
    ...
    model_name = os.path.join(args.prefix + '_' + '_'.join(
    ...
    train = 'CUDA_VISIBLE_DEVICES=' + args.gpu + ' python models/train_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -b 6 -clip 5 -p 100 -hidden_dim 512 -pooling_dim 4096 -ngpu 1 -ckpt checkpoints/vgdet/vg-24.tar -nepoch 50 -use_bias' \
         + ' ' + arguments + ' ' \
